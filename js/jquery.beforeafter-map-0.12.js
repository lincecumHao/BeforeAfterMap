...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			_bID_ = _before_.getDiv().getAttribute('id'), _aID_ = _after_.getDiv().getAttribute('id');
			
			// edit 2015/03/06
			google.maps.event.addListener(_before_, 'idle', function() { _after_.panTo(_before_.getCenter());});
			google.maps.event.addListener(_after_, 'idle', function() { _before_.panTo(_after_.getCenter()); });
			...
			console.log(_before_._container);
			$(_before_.getDiv()).height(mapHeight).width(mapWidth*o.introPosition).css({'position':'absolute','overflow':'hidden','left':'0px','z-index':'10'}); // Set CSS properties of the before map div
			$(_after_.getDiv()).height(mapHeight).width(mapWidth).css({'position':'absolute','overflow':'hidden','right':'0px'});	// Set CSS properties of the after map div
			...
			$(_before_.getDiv()).css({'position':'absolute','top':'0px','left':'0px'});
			$(_after_.getDiv()).css({'position':'absolute','top':'0px','right':'0px'});
			...
			if (o.changeOnResize){
				...
				$(window).on('orientationchange pageshow resize', function () {
					var id = o.thisID;
					w = $(obj).width();
					$(_before_.getDiv()).width(w);
					$(_after_.getDiv()).width(w);
					google.maps.event.trigger(_before_, "resize");
					google.maps.event.trigger(_after_, "resize");
					
					clearInterval(cInt);
					cInt = setInterval(function(){
						$(_before_.getDiv()).width(parseInt( $('#dragwrapper'+randID).css('left') ) + 4 );
						if ($(_before_.getDiv()).width() != w){ clearInterval(cInt); }
					}, 100);
				});
			}
			...
			google.maps.event.addListener(_after_, 'idle', _mapMove_);
			
			function _mapMove_(e)
			{
				var zoom = this.getZoom(),
				latlng = this.getCenter();
				var thisID = this.getDiv().getAttribute('id')
				if(thisID == _bID_){
					
					_after_.panTo(latlng);
					_after_.setZoom(zoom);
				}else if(thisID == _aID_){
					
					_before_.panTo(latlng);
					_before_.setZoom(zoom);
